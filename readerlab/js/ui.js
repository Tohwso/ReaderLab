    ...
    <div style="display:flex;flex-direction:column;gap:10px">
      ${result.reactions.map((rr) => {
        ...
        return `<div class="card" style="padding:13px 16px">
          ...
          ${rr.reason ? `<p class="muted small">${esc(rr.reason)}</p>` : ""}
        </div>`;
      }).join("")}
    </div>` : "";

  const answersHTML = result && result.surveyAnswers.length ? `
    <div class="section-title">Respostas da pesquisa</div>
    <div style="display:flex;flex-direction:column;gap:10px">
      ${result.surveyAnswers.map((ans) => {
        const q = survey ? survey.questions.find((x) => x.id === ans.questionId) : null;
        const label = q ? q.text : `(pergunta removida: ${ans.questionId})`;
        let valueHTML;
        if (q && (q.type === "scale" || q.type === "number")) {
          const pct = q.max > q.min ? Math.round(((ans.value - q.min) / (q.max - q.min)) * 100) : 0;
          valueHTML = `<div class="mono">${ans.value} / ${q.max}</div><div class="bar" style="margin-top:5px"><i style="width:${pct}%"></i></div>`;
        } else if (q && q.type === "boolean") {
          valueHTML = `<div>${ans.value ? "Sim" : "Não"}</div>`;
        } else {
          valueHTML = `<p style="white-space:pre-wrap">${esc(String(ans.value))}</p>`;
        }
        return `<div class="card" style="padding:13px 16px">
          <p style="font-weight:650;margin-bottom:6px">${esc(label)}</p>
          <div class="muted">${valueHTML}</div>
        </div>`;
      }).join("")}
    </div>` : "";

  const notesHTML = result && result.spontaneousNotes.length ? `
    <div class="section-title">Notas espontâneas</div>
    <div class="card"><ul style="margin:0;padding-left:18px">
      ${result.spontaneousNotes.map((n) => `<li style="margin:4px 0">${esc(n)}</li>`).join("")}
    </ul></div>` : "";

  let readerHTML = "";
  if (result && result.readerState && typeof result.readerState === "object") {
    const rs = result.readerState;
    const chips = [];
    for (const k of ["engagement", "curiosity", "fatigue"]) {
      if (typeof rs[k] === "number") chips.push(`<span class="badge neutral">${k}: ${rs[k]}</span>`);
    }
    for (const k of ["confusions", "predictions"]) {
      if (Array.isArray(rs[k]) && rs[k].length) chips.push(`<span class="badge neutral">${k}: ${rs[k].map((x) => esc(String(x))).join("; ")}</span>`);
    }
    if (chips.length) {
      readerHTML = `<div class="section-title">Estado do leitor</div>
        <div class="card"><div class="tags">${chips.join(" ")}</div>
        <p class="hint" style="margin-top:8px">Estado pontual desta execução — memória longitudinal entre capítulos fica para uma fase posterior.</p></div>`;
    }
  }

  main.innerHTML = `
    ${pageHead(esc(run.title || "Resultado da leitura"), "",
      `<a class="btn" href="#/execucoes">Voltar ao histórico</a>`)}
    <div class="card" style="margin-bottom:18px">
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px 18px">
        ...
        <div><div class="faint small">DURAÇÃO</div><span class="mono">${fmtDuration(run.startedAt, run.completedAt)}</span></div>
      </div>
    </div>

    ${run.status === "FAILED" ? `
      <div class="info-box warn" style="margin-bottom:16px"><span>⚠</span>
        <span><b>Execução falhou.</b> ${esc(run.errorMessage || "Erro não detalhado.")}</span>
      </div>` : ""}
    ...
    ${persona && run.status !== "FAILED" ? `
      <div class="section-title">Persona utilizada</div>
      <div class="card" style="margin-bottom:6px">
        <div class="tags" style="margin-bottom:8px">${(persona.tags || []).map((t) => `<span class="tag">${esc(t)}</span>`).join(" ")}</div>
        <p class="muted">${esc(persona.shortDescription || "")}</p>
      </div>` : ""}
    ...
    <div style="display:flex;flex-direction:column;gap:10px">
      <details class="card"><summary style="cursor:pointer;font-weight:650">Ver resposta JSON</summary>
        ${run.rawResponse
